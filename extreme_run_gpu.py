...
X_raw = df.drop(columns=[TARGET_COL]).reset_index(drop=True)
# Save processed feature matrix for Objective-2
X_raw.to_csv("./output/final_raw_X.csv", index=False)
pd.Series(y).to_csv("./output/final_labels.csv", index=False)

print("Saved final_raw_X.csv and final_labels.csv")
...
def preprocess_fold(X_train, X_test):
    ...
    if cat_cols:
        ...
        Xtr_enc = enc.fit_transform(Xtr[cat_cols])
        Xte_enc = enc.transform(Xte[cat_cols])

        Xtr_enc = pd.DataFrame(
            Xtr_enc,
            columns=enc.get_feature_names_out(cat_cols),
            index=Xtr.index
        )

        Xte_enc = pd.DataFrame(
            Xte_enc,
            columns=enc.get_feature_names_out(cat_cols),
            index=Xte.index
        )
        ...
        Xtr = pd.concat(
            [Xtr.drop(columns=cat_cols), Xtr_enc],
            axis=1
        )

        Xte = pd.concat(
            [Xte.drop(columns=cat_cols), Xte_enc],
            axis=1
        )
...
def xgb_cv_score(params, Xdf, yarr, folds=3, seed=SEED):
    ...
    for tr, te in skf_local.split(Xdf, yarr):
        Xtr_raw = Xdf.iloc[tr].copy()
        Xte_raw = Xdf.iloc[te].copy()
...
def optuna_objective(trial):
    ...
    # compute score
    score = xgb_cv_score(p, X_raw, y, folds=3, seed=SEED)
...
def de_obj(vec):
    try:
        ...
        score = xgb_cv_score(p, X_raw, y, folds=3)
...
def evaluate_candidate_with_stats(params, folds=3):
    ...
    skf_local = StratifiedKFold(n_splits=folds, shuffle=True, random_state=SEED)
    accs = []
    for tr,te in skf_local.split(X_raw, y):
        Xtr_raw = X_raw.iloc[tr].copy()
        Xte_raw = X_raw.iloc[te].copy()
...
de_score = xgb_cv_score(de_refined, X_raw, y, folds=3)
...
X_final, _ = preprocess_fold(X_raw, X_raw)

final_xgb.fit(X_final.values, y)
...
for lr in lr_values:
    ...
    acc = xgb_cv_score(p, X_raw, y, folds=3)
...
# original
orig_acc = xgb_cv_score(XGB_TUNED_PARAMS, X_raw, y, folds=3)
robust_results.append({"scenario":"original","accuracy":orig_acc})

# noise
noise = np.random.normal(0,0.01,X_raw.shape)
X_noise = X_raw.copy()

num_cols = X_noise.select_dtypes(include=[np.number]).columns

X_noise[num_cols] = X_noise[num_cols] + noise[:, :len(num_cols)]
...
num_cols = X_missing.select_dtypes(include=[np.number]).columns
...
def eval_cat(Xtr, ytr, Xte, yte):
    ...
    return float(accuracy_score(yte, preds)), preds

def eval_tabnet(Xtr, ytr, Xte, yte):
    ...
    if len(missing):
        ...
        dummy = np.zeros(
            (len(missing), Xtr.shape[1])
        )

        Xtr_aug = np.vstack([Xtr.values, dummy])
        ytr_aug = np.concatenate([ytr, missing])
...
rf_scores = []; xgb_scores = []; lgb_scores = []; cat_scores = []; tab_scores = []
...
for f, (tr, te) in enumerate(skf.split(X_raw, y), 1):
    print(f"\n===== FOLD {f}/{N_FOLDS} =====")
    t0 = time.time()
    Xtr_raw = X_raw.iloc[tr].reset_index(drop=True)
    Xte_raw = X_raw.iloc[te].reset_index(drop=True)
    ...
    # CATBOOST
    try:
        cat_acc, cat_preds = eval_cat(Xtr, ytr, Xte, yte)
    ...
    # TABNET
    try:
        tab_acc, tab_preds = eval_tabnet(Xtr, ytr, Xte, yte)
        tab_scores.append(tab_acc)
        print("TAB ✓", tab_acc)
    except Exception as e:
        print("TAB fail:", e)
        tab_scores.append(np.nan)
...
cat_mean = float(np.nanmean(cat_scores)) if len(cat_scores) else 0.0
tab_mean = float(np.nanmean(tab_scores)) if len(tab_scores) else 0.0
...
print(f"CatBoost mean: {cat_mean:.4f}")
print(f"TabNet mean  : {tab_mean:.4f}")
overall_mean = np.nanmean([rf_mean, xgb_mean, lgb_mean, cat_mean, tab_mean])
print(f"Overall Mean Accuracy (model-average) ≈ {overall_mean:.4f}")

summary = pd.DataFrame({"RF": rf_scores, "XGB": xgb_scores, "LGB": lgb_scores, "CAT": cat_scores, "TAB": tab_scores})
...
tab_arr = np.array(tab_scores)

# Ensure equal length for tests
min_len = min(len(rf_arr), len(xgb_arr), len(lgb_arr), len(cat_arr), len(tab_arr))
rf_arr = rf_arr[:min_len]
xgb_arr = xgb_arr[:min_len]
lgb_arr = lgb_arr[:min_len]
cat_arr = cat_arr[:min_len]
tab_arr = tab_arr[:min_len]

valid_mask = ~np.isnan(tab_arr)

rf_arr = rf_arr[valid_mask]
xgb_arr = xgb_arr[valid_mask]
lgb_arr = lgb_arr[valid_mask]
cat_arr = cat_arr[valid_mask]
tab_arr = tab_arr[valid_mask]

# -------- Friedman Test --------
stat, p = friedmanchisquare(rf_arr, xgb_arr, lgb_arr, cat_arr, tab_arr)
...
w_cat = wilcoxon(xgb_arr, cat_arr).pvalue
w_tab = wilcoxon(xgb_arr, tab_arr).pvalue

print(f"EvoX vs RF p-value        : {w_rf:.6f}")
print(f"EvoX vs LightGBM p-value  : {w_lgb:.6f}")
print(f"EvoX vs CatBoost p-value  : {w_cat:.6f}")
print(f"EvoX vs TabNet p-value    : {w_tab:.6f}")
