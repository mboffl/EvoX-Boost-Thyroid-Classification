...
def plot_convergence():
    ...
    path = os.path.join(OUT, "convergence_history.csv")

    if not os.path.exists(path):
        print("[conv] convergence_history.csv missing")
        return
    ...
    sns.lineplot(data=df, x="iteration", y="score", marker="o", ax=ax)

    ax.set_xlabel("Iteration")
    ax.set_ylabel("Validation Accuracy")
    ax.set_title("Optimization Convergence")

    save_fig(fig, "fig_1_convergence_curve.png")
    print("[conv] saved")
...
def plot_ablation():
    ...
    path = os.path.join(OUT, "ablation_results.csv")

    if not os.path.exists(path):
        print("[ablation] file missing")
        return
    ...
    sns.barplot(data=df, x="stage", y="accuracy", palette="viridis", ax=ax)
    ...
    ax.set_title("Ablation Study of EvoX Optimization")

    save_fig(fig, "fig_ablation_study.png")
...
def plot_optimization_runtime():

    path = os.path.join(OUT, "optimization_runtime.csv")

    if not os.path.exists(path):
        print("[runtime] optimization_runtime.csv missing")
        return
    ...
    sns.barplot(data=df, x="method", y="time_seconds", palette="mako", ax=ax)
    ...
    save_fig(fig, "fig_optimization_runtime.png")
...
def plot_sensitivity():
    ...
    path = os.path.join(OUT, "sensitivity_results.csv")

    if not os.path.exists(path):
        print("[sens] sensitivity_results.csv missing")
        return
    ...
    sns.lineplot(data=df, x="value", y="accuracy", marker="o", ax=ax)

    ax.set_xlabel("Learning Rate")
    ax.set_ylabel("Accuracy")
    ax.set_title("Hyperparameter Sensitivity Analysis")

    save_fig(fig, "fig_sensitivity_analysis.png")
...
def plot_robustness():
    ...
    path = os.path.join(OUT, "robustness_results.csv")

    if not os.path.exists(path):
        print("[robust] robustness_results.csv missing")
        return
    ...
    sns.barplot(data=df, x="scenario", y="accuracy", palette="crest", ax=ax)
    ...
    ax.set_title("Model Robustness Evaluation")

    save_fig(fig, "fig_robustness_test.png")
...
def plot_corr():
    ...
    fig, ax = plt.subplots(figsize=(16, 12))  # crisp & readable

    sns.heatmap(
        corr,
        cmap="Blues",         # 💙 clean blue theme
        annot=False,           # show all values
        linewidths=0.4,
        linecolor="black",
        cbar_kws={"shrink": 0.5},
        ax=ax
...
def plot_confusion_and_metrics(labels):
    ...
    for name, pref in models:
        ...
        ax.set_xticklabels(range(cm.shape[1]), fontsize=9)
        ax.set_yticklabels(range(cm.shape[0]), fontsize=9)
...
def plot_accuracy_comparison():
    ...
    labels = [display_names[m] for m in means.keys()]
    ...
    bars = ax.bar(
        labels,
        values,
        color=sns.color_palette("tab10", len(values))
    )
    ...
    ax.set_ylabel("Accuracy")
    ax.set_title("Performance Comparison of Baseline Models")

    plt.xticks(rotation=15)

    save_fig(
        fig,
        "fig_6_accuracy_comparison.png"
    )
